...
{
    /// <summary>警告コールバックを呼んで続行する。既定。</summary>
    Warn,
...
public sealed class ChunkReadOptions
{
    ...
    /// <summary>警告の通知先。null なら何もしない。</summary>
...
public sealed class Chunk
{
    ...
    /// <summary>最下段セクションのY位置。オーバーワールドは -4。</summary>
    ...
    /// <summary>生成が完了しているか。ブロック改変の対象にしてよいのはこれだけ。</summary>
    public bool IsFullyGenerated => Status == "minecraft:full";

    /// <summary>存在するセクションのY位置。昇順。</summary>
    ...
    /// <summary>Y位置からセクションを得る。無ければ null。</summary>
    ...
    /// <returns>ブロック。セクションが無い、または block_states を持たない場合は null。</returns>
    ...
    /// </summary>
    /// <returns>バイオームID。セクションが無い場合は null。</returns>
    ...
    /// <summary>バイオームを設定する。4×4×4 の単位。</summary>
    ...
    /// <summary>セクション内のバイオーム添字。1 エントリが 4×4×4 ブロック。</summary>
