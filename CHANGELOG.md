...
最初のリリース。対象は **26.1 で導入されたワールド形式**（DataVersion 4786 以降）。
動作は Java版 26.2（DataVersion 4903）の実ワールドで確かめている。
...
- 全言語で一致する `ErrorCode`
- ワールド形式による対応判定。下限（26.1）以降なら警告を出さず、
  書き戻すときも `DataVersion` を書き換えない（[adr/0003](docs/adr/0003-version-policy.md)）
