...
| 3 | 無圧縮 | 読み書き対応 |
| 4 | LZ4（独自ヘッダ付きのブロック連結） | **読み込みのみ対応**。書き込みは `UNSUPPORTED_FEATURE` |
...
ブロック本体（トークンが `0x20` の場合）は素の LZ4 ブロック形式である。
**シーケンスの並び**で、各シーケンスは次の形をとる。

```
+0   1 バイト   トークン
                 上位 4 ビット: リテラル長 (0..15)
                 下位 4 ビット: マッチ長 - 4 (0..15)
     0..n バイト リテラル長の追加バイト（上位 4 ビットが 15 のときだけ）
     n バイト    リテラル（そのまま出力へ）
--- ここで入力を使い切ったらシーケンスは終わり ---
     2 バイト    マッチのオフセット  u16 **リトルエンディアン**
     0..n バイト マッチ長の追加バイト（下位 4 ビットが 15 のときだけ）
```

追加バイトは **255 が続く限り読み進め、255 未満のバイトで終わる**。
読んだバイトの総和を基本の長さへ足す。

```
リテラル長 = 15 + 追加バイトの総和
マッチ長   = 4 + (トークンの下位 4 ビット) + 追加バイトの総和
```

マッチは**出力済みのバイト列から**コピーする。
コピー開始位置は「現在の出力長 - オフセット」である。

**コピーは 1 バイトずつ行うこと。** コピー元とコピー先は重なりうる。
たとえばオフセット 1 でマッチ長 10 なら、直前の 1 バイトが 10 回並ぶ。
まとめてコピーする実装（`memcpy` など）では結果が変わる。

展開の終わり方は次のとおり。

- リテラルを出力したあと入力が尽きていれば、そのシーケンスで終わり
- 尽きていなければオフセットを読み、マッチをコピーして次のシーケンスへ進む

検証項目:

| 条件 | 結果 |
|---|---|
| オフセットが `0` | `MALFORMED_DATA` |
| オフセットが現在の出力長より大きい | `MALFORMED_DATA` |
| 入力の終端を越えて読もうとした | `MALFORMED_DATA` |
| 出力が展開後の長さを超えた | `MALFORMED_DATA` |
| 展開しきった長さが展開後の長さと違う | `MALFORMED_DATA` |
