...
| `player` | `Player()` | `player()` | `player()` | `player()` | `player()` | プレイヤーデータを読む。 |
...
| `writable` | `Writable` | `writable()` | `writable` | `writable` | `writable` | 読み書きで開くか。 |
| **LevelData** | `LevelData` | `LevelData` | `LevelData` | `LevelData` | `LevelData` | level.dat の内容。 |
| `data` | `Data` | `data()` | `data` | `data` | `data()` | Data の中身。 |
...
| `game_type` | `GameType` | `gameType()` | `gameType()` | `game_type` | `game_type()` | ゲームモード。 |
...
| `raw` | `Raw` | `raw()` | `raw` | `raw` | `raw()` | ルートの NBT。 |
...
| `entity_folder` | `EntityFolder` | `entityFolder()` | `entityFolder()` | `entity_folder()` | `entity_folder()` | エンティティのリージョンフォルダ。 |
| `flush` | `Flush()` | `flush()` | `flush()` | `flush()` | `flush()` | 変更したチャンクをすべて書き戻し、リージョンをディスクへ反映する。 |
| `get_biome` | `GetBiome()` | `getBiome()` | `getBiome()` | `get_biome()` | `get_biome()` | 絶対座標でバイオームを取得する。 |
...
| `poi_folder` | `PoiFolder` | `poiFolder()` | `poiFolder()` | `poi_folder()` | `poi_folder()` | POI のリージョンフォルダ。 |
| `region_folder` | `RegionFolder` | `regionFolder()` | `regionFolder()` | `region_folder()` | `region_folder()` | 地形のリージョンフォルダ。 |
| `save_chunk` | `SaveChunk()` | `saveChunk()` | `saveChunk()` | `save_chunk()` | — | チャンクを書き戻す。 |
| `set_biome` | `SetBiome()` | `setBiome()` | `setBiome()` | `set_biome()` | `set_biome()` | 絶対座標でバイオームを設定する。 |
| `set_block` | `SetBlock()` | `setBlock()` | `setBlock()` | `set_block()` | `set_block()` | 絶対座標でブロックを設定する。 |
| **Chunk** | `Chunk` | `Chunk` | `Chunk` | `Chunk` | `Chunk` | チャンク 1 つ分。 |
| `biome_index` | `BiomeIndex()` | `biomeIndex()` | `biomeIndex()` | `biome_index()` | `biome_index()` | セクション内のバイオーム添字。 |
...
| `is_fully_generated` | `IsFullyGenerated` | `isFullyGenerated()` | `isFullyGenerated()` | `is_fully_generated` | `is_fully_generated()` | 生成が完了しているか。 |
| `min_section_y` | `MinSectionY` | `minSectionY()` | `minSectionY()` | `min_section_y` | `min_section_y()` | 最下段セクションのY位置。 |
| `raw` | `Raw` | `raw()` | `raw` | `raw` | `raw()` | 元の NBT。 |
| `section` | `Section` | `section()` | `section()` | `section()` | `section()` | Y位置からセクションを得る。 |
| `section_mut` | — | — | — | — | `section_mut()` |  |
| `section_ys` | `SectionYs` | `sectionYs()` | `sectionYs()` | `section_ys` | `section_ys()` | 存在するセクションのY位置。 |
| `set_biome` | `SetBiome()` | `setBiome()` | `setBiome()` | `set_biome()` | `set_biome()` | バイオームを設定する。 |
...
| `biomes` | `Biomes` | `biomes()` | `biomes()` | `biomes` | `biomes()` | バイオーム。 |
| `biomes_mut` | — | — | — | — | `biomes_mut()` |  |
| `block_states` | `BlockStates` | `blockStates()` | `blockStates()` | `block_states` | `block_states()` | ブロック状態。 |
...
| `on_warning` | `OnWarning` | `onWarning()` | `onWarning` | `on_warning` | `on_warning` | 警告の通知先。 |
...
| `warn` | `Warn` | `WARN` | `Warn` | `warn` | `warn` | 警告コールバックを呼んで続行する。 |
...
| `properties` | `Properties` | `properties()` | `properties()` | `properties` | `properties()` | プロパティ。 |
| `property` | `Property()` | `property()` | `property()` | `property()` | `property()` | プロパティを取得する。 |
...
| `ceil_log2` | `CeilLog2()` | `ceilLog2()` | `ceilLog2()` | `ceil_log2()` | `ceil_log2()` | 個の値を表すのに必要な最小ビット数。 |
| `compact` | `Compact()` | `compact()` | `compact()` | `compact()` | `compact()` | どのエントリからも参照されていないパレット要素を取り除き、添字を振り直す。 |
| `entry_count` | `EntryCount` | `entryCount()` | `entryCount()` | `entry_count` | `entry_count()` | エントリ数。 |
| `fill` | `Fill()` | `fill()` | `fill()` | `fill()` | `fill()` | 全エントリを 1 つの値で埋める。 |
...
| `min_bits` | `MinBits` | `minBits()` | `minBits()` | `min_bits` | `min_bits()` | ビット幅の下限。 |
| `palette` | `Palette` | `palette()` | `palette()` | `palette` | `palette()` | パレット。 |
...
| `set` | `Set()` | `set()` | `set()` | `set()` | `set()` | 添字の値を書き換える。 |
| `to_longs` | `ToLongs()` | `toLongs()` | `toLongs()` | `to_longs()` | — | packed な配列を返す。 |
