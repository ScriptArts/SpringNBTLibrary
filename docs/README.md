...
違いは、同じ設計者が、同じ仕様書と同じテストベクタから
全言語分を同時に作っている点です。

そのために次を守っています。

1. `docs/spec/` が唯一の正であり、各言語はその実装にすぎない
2. 挙動の一致は目視ではなく[適合性テスト](spec/90-conformance.md)で確かめる
3. [機能一覧](features.md)が実装からずれないよう CI で検査する

Rust のツールと Java のプラグインで同じワールドを触るときに、
