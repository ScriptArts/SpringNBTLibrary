...
SpringNBTLibrary が提供する機能と、各言語の対応状況。
...
|---|---|
| ✅ | 対応済み |
| 🔶 | 任意依存を入れた場合のみ対応（未導入時は `UNSUPPORTED_FEATURE`） |
| ⏳ | 実装中 |
| ❌ | 未対応（右端の「備考」に理由と予定を記載） |

対応言語は **C# / Java / TypeScript / Python / Rust** の5つ。
対応バージョンは **Java版 26.2 / DataVersion 4903**。
過去バージョンのワールド改変は非サポート（→ [07 バージョンポリシー](guide/07-version-policy.md)）。

この表は CI の `docs-sync` ジョブで各言語の公開APIと突き合わせており、
表と実装が食い違うとビルドが失敗する。

**実データでの検証**: レイヤ1（NBT）とレイヤ2（Anvil）は Java版 26.2 の実ワールドを走査し、
`.dat` 23個 + チャンク 3,717個 = **3,740件すべてで読み込みとバイト一致のラウンドトリップに成功**している。
リージョンファイル 12 個は**開いて無変更で書き戻すとバイト単位で原本と一致**する。
さらに代表ファイルを5言語すべてに通し、正規化JSON・SNBT・チャンク一覧・詰め直したバイト列が
**全言語で完全一致**することも確認済み（→ [90 適合性 2.4](spec/90-conformance.md#24-実ワールド走査)）。
...
| 圧縮ID 4 (LZ4) | ❌ | ❌ | ❌ | ❌ | ❌ | [20](spec/20-anvil-region.md#31-圧縮方式id) | 生バイトAPIでのみ取得可。v0.2.0 予定 |
...
| `Heightmaps` の再計算 | ❌ | ❌ | ❌ | ❌ | ❌ | [30](spec/30-chunk-format.md#3-heightmaps) | ブロック定義テーブルが必要。v0.2.0 予定（[adr/0004](adr/0004-defer-heightmap-recalc.md)） |
| 光源の再計算 | ❌ | ❌ | ❌ | ❌ | ❌ | [30](spec/30-chunk-format.md#3-heightmaps) | 同上。`invalidate_lighting()` で無効化のみ提供 |
| `entities` / `poi` の型付きAPI | ❌ | ❌ | ❌ | ❌ | ❌ | [30](spec/30-chunk-format.md#4-entities--poi) | 生NBTとしてなら読み書き可。v0.2.0 予定 |
...
同一の概念・機能を提供しているが、言語の性質上どうしても表現が変わる箇所がある。
移植時に迷わないよう、ここに集約しておく。
