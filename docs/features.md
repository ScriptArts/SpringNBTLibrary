...
`.dat` 23個 + チャンク 3,717個 = 3,740件すべてで読み込みとバイト一致のラウンドトリップに成功している。
リージョンファイル 12 個は開いて無変更で書き戻すとバイト単位で原本と一致する。
さらに代表ファイルを対応言語すべてに通し、正規化JSON・SNBT・チャンク一覧・詰め直したバイト列が
全言語で完全一致することも確認済み（→ [90 適合性 2.4](spec/90-conformance.md#24-実ワールド走査)）。
