...
対象は Minecraft Java版 26.2（DataVersion 4903）です。
...
既定では `UNSUPPORTED_DATA_VERSION` で止まります。
...
> 必ずワールドのバックアップを取ってから使ってください。

安全な手順は「先に Minecraft で一度ワールドを開いて
公式のアップグレードを済ませ、そのあとこのライブラリで編集する」です。
...
Minecraft が更新されても、こちらが対応するまでは
そのワールドを書き換えないでください。
読むだけなら警告つきで通りますが、
書き戻すと新しい構造を古い解釈で上書きしかねません。
