...
バージョンごとの構造差をすべて実装し、そのすべてを検証し続けることになる。

1人で対応言語すべてを維持しながら、それを正しく保つのは不可能である。
中途半端に対応すると、**利用者のワールドを静かに壊す**。

そこで「対象は 1 バージョンだけ」と決め、そのぶん正しさに投資している。

---

## 2. 読み込み

**DataVersion が何であっても、構造が解釈できる限り読む。**
...
バージョンに関わらず `MALFORMED_DATA` になる。**黙って推測はしない。**

### NBT レイヤはバージョンに依存しない

[レイヤ1（NBT）](01-nbt.md)と[レイヤ2（Anvil）](03-anvil-region.md)は
バージョン検査を一切しない。
どのバージョンのファイルでも読み書きできる。

バージョンを見るのは[レイヤ3（World）](04-world-and-level-dat.md)だけである。
古いワールドから NBT を取り出すだけなら、何の制約もなく使える。

---

## 3. 書き込み

**書き戻すチャンクの `DataVersion` は常に 4903 になる。**
...
対応の手順は [リリース手順](../contributing/release.md) にまとめてある。
